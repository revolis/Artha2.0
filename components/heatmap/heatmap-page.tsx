...
// Per-day aggregation for the tooltip: net decides the color, the rest fills
// the hover details.
interface DayStats {
  profit: number
  loss: number
...
export function HeatmapPage() {
  ...
  // Aggregate entries into per-day stats for the selected year.
  ...
  const maxAbsNet = Math.max(
    0,
    ...Array.from(dayStats.values()).map((s) => Math.abs(s.net))
  )

  // GitHub-style grid: columns are weeks (starting Sunday), rows are weekdays.
          ...
          <div className="flex items-center justify-end gap-2 text-xs text-muted-foreground">
            ...
            <div className="flex gap-0.5">
              {[...redLevels].reverse().map((cls) => (
                <div key={cls} className={cn("size-3.5 rounded-[3px] border", cls)} />
              ))}
            </div>
